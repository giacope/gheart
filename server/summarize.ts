...
/**
 * Ask Claude for a tldr + eli5. Falls back to the heuristic on any error so the
 * deck always loads.
 */
export async function summarize(input: SummaryInput): Promise<Summary> {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return heuristicSummary(input);
  ...
  const prompt = [
    'Summarize this GitHub pull request for a reviewer deciding whether to approve it.',
    'Respond with ONLY a JSON object: {"tldr": string, "eli5": string, "tags": string[]}.',
    '- tldr: 1-2 sentences, technical, what changed and why.',
    '- eli5: one sentence a non-programmer would understand, friendly tone.',
    '- tags: 2-4 short lowercase labels (e.g. "bugfix", "ui", "risky", "quick-read").',
    '',
    `Title: ${input.title}`,
    `Stats: +${input.additions}/-${input.deletions} across ${input.changedFiles} files`,
    `Files:\n${fileList || '(none listed)'}`,
    `Description:\n${input.body.slice(0, 4000) || '(empty)'}`,
  ].join('\n');
  ...
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: process.env.GHEART_MODEL || 'claude-haiku-4-5-20251001',
        max_tokens: 400,
        messages: [{ role: 'user', content: prompt }],
      }),
    });
    if (!res.ok) throw new Error(`Claude API ${res.status}`);
    const data = (await res.json()) as ClaudeContent;
    const text = data.content?.find((c) => c.type === 'text')?.text ?? '';
    const jsonMatch = text.match(/\{[\s\S]*\}/);
    if (!jsonMatch) throw new Error('no JSON in response');
    const parsed = JSON.parse(jsonMatch[0]) as Partial<Summary>;
    if (typeof parsed.tldr !== 'string' || typeof parsed.eli5 !== 'string') {
