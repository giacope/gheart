...
describe('Store', () => {
  ...
  it('upserts users and preserves createdAt across updates', () => {
    const first = store.upsertUser(alice, 'tok-1');
    const second = store.upsertUser({ ...alice, name: 'Alice B' }, 'tok-2');
  ...
  it('keeps swipe history separate per user', () => {
    store.upsertUser(alice, 't1');
    store.upsertUser(bob, 't2');
